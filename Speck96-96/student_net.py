...
def train_speck_distinguisher(num_epochs, num_rounds=7, depth=1, diff=(0x80, 0x0), bits=[11,10,9,8], teacher='./', folder='./'):
    ...
    # np.save(wdir+'h'+str(num_rounds)+'r_depth'+str(depth)+'.npy', h.history['val_mse'])
    np.save(wdir+'h'+str(num_rounds)+'r_depth'+str(depth)+'.npy', h.history['val_loss'])
    dump(h.history, open(wdir+'hist'+str(num_rounds)+'r_depth'+str(depth)+'.p', 'wb'))
    # print("Best validation mse: ", np.min(h.history['val_mse']))

    # save model
    net.save(folder + '/soft_label/student_' + str(num_rounds) + '_distinguisher.h5')
